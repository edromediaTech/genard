<template>
  <v-container>
    ...
    <v-stepper v-model="step">
      <!-- En-tête du stepper -->
      <v-stepper-header>
        <v-stepper-step :complete="step > 1" step="1">Dates et Chambre</v-stepper-step>
        <v-divider></v-divider>
        <v-stepper-step :complete="step > 2" step="2">Informations du Client</v-stepper-step>
        <v-divider></v-divider>
          ...
          <v-card class="pa-4">
            ...
            <v-form ref="bookingForm" v-model="formValid">
              <v-row>
                ...
                <v-col cols="12" md="6" sm="6">
                  <v-btn small color="primary" class="mb-4" @click="openAddClientDialog" >
                    <v-icon left>mdi-plus</v-icon>Ajouter un nouveau client
                  </v-btn>
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12" md="4" sm="4">
                  <v-text-field v-model="clientUser.name" label="Nom du client" outlined required></v-text-field>
                </v-col>
                <v-col cols="12" md="4" sm="4">
                  <v-text-field v-model="clientUser.email" label="Email du client" outlined required></v-text-field>
                </v-col>
                <v-col cols="12" md="4" sm="4">
                  <v-text-field v-model="clientUser.phoneNumber" label="Numéro de téléphone" outlined required></v-text-field>
                </v-col>
              </v-row>
              <h3 class="mb-4">Occupants supplémentaires</h3>
              <v-row v-for="(occupant, index) in occupants" :key="index">
                <v-col cols="12" md="3">
                  <v-text-field v-model="occupant.name" label="Nom de l'occupant" outlined></v-text-field>
                </v-col>
                <v-col cols="12" md="3">
                  <v-text-field v-model="occupant.age" label="Âge" type="number" outlined></v-text-field>
                </v-col>
                <v-col cols="12" md="3">
                  <v-text-field v-model="occupant.identity" label="No identité" type="number" outlined></v-text-field>
                </v-col>
                <v-col cols="12" md="3">
                  <v-select v-model="occupant.gender" :items="['male', 'female', 'other']" label="Sexe" outlined></v-select>
                </v-col>
                <v-col cols="12" md="1">
              ...
              <v-btn class="mb-4" @click="addOccupant" >
                <v-icon left>mdi-plus</v-icon>Ajouter un occupant
              </v-btn>
              <v-text-field v-model="depot" label="Dépot" type="number" outlined></v-text-field>
            </v-form>
            <v-btn color="warning" class="mr-2" @click="goBackToStep1">Précédent</v-btn>
            <v-btn :disabled="!formValid" color="success" @click="goToStep3">Suivant</v-btn>
          </v-card>
        </v-stepper-content>

        <!-- Étape 3 : Récapitulatif et impression -->
              ...
              <v-card-text>
                ...
                <ul>
                  <li v-for="(occupant, index) in occupants" :key="index">
                    {{ occupant.name }} ({{ occupant.age }} ans, {{ occupant.gender }}, {{ occupant.identity }})
                  </li>
                </ul>
              </v-card-text>
              <v-card-actions>
                <!-- <v-btn color="primary" @click="generateReport">
                  <v-icon left>mdi-file-pdf</v-icon>Générer PDF
                </v-btn> -->
              
                <v-btn color="success" @click="confirmBooking">Confirmer</v-btn>
              </v-card-actions>
            </v-card>
     ...
     <client-only>
      ...
      >
        <template slot="pdf-content">
          <booking-printer :booking="bookingData" />
        </template>
      </vue-html2pdf>
    </client-only>
  </v-container>
...
export default {
  ...
  methods: {
   ...
   generateReport() {
      // Préparer les données de réservation pour le PDF
      this.bookingData = {
        roomNumber: this.selectedRoomDetails.roomNumber,
        roomType: this.selectedRoomDetails.type,
        checkInDate: this.checkInDate,
        checkOutDate: this.checkOutDate,
        clientName: this.clientUser.name,
        clientEmail: this.clientUser.email,
        occupants: this.occupants,
        totalPrice: this.calculateTotalPrice(), // Méthode à implémenter
      };
    ...
    calculateTotalPrice() {
      // Calculez le prix total en fonction des dates et du prix de la chambre
      const nights = Math.ceil(
        (new Date(this.checkOutDate) - new Date(this.checkInDate)) / (1000 * 60 * 60 * 24)
      );
      return nights * this.selectedRoomDetails.price;
    },
    ...
    addOccupant() {
      this.occupants.push({ name: '', age: null, gender: '', identity:'' });
    },
   ...
   async confirmBooking() {
      try {
        ...
        console.log(this.occupants, this.clientUser)
         await this.$axios.post('/bookings', bookingData);
        this.generateReport()
        this.$notifier.showMessage({ content: "Réservation confirmée avec succès !", color: "success", });
